...
{
    Bitboard attackedByEnemy = checkLine(&white); //get line from which there is a check
    Bitboard kingMoves;
    isMatWhite=false;
    isMatBlack=false;
    isCheckedBlack=false;
    ...
    if(attackedByEnemy!=0)
    {
        ...
        isCheckedWhite=true;
        kingMoves = moveGeneratorKing(white.king, &white);
        if((getAttackedFields(&white) & attackedByEnemy) == 0 && ((kingMoves^attackedByEnemy & kingMoves) == 0))
        {
            isMatWhite=true;
            return;
        }
    ...
    if(attackedByEnemy!=0)
    {
        ...
        isCheckedBlack=true;
        kingMoves = moveGeneratorKing(black.king, &black);
        if((getAttackedFields(&black) & attackedByEnemy) == 0 && ((kingMoves^attackedByEnemy & kingMoves) == 0))
        {
            isMatBlack=true;
            return;
        }
